...
<section class="content">
    <div class="box">
        <div class="box-body" style="margin-top: 20px;">
            <form action="<?= base_url('supplier/processAdd')?>" method="post">
                <div class="form-group has-feedback <?= form_error('name') ? "has-error" : "null" ?>">
                    <label for="name">Name Supplier :</label>
                    <input type="text" name="name" id="name" value="<?= set_value('name')?>" class="form-control" autocomplete="off" placeholder="Masukan Name Supplier">
                ...
                <div class="form-group <?= form_error('phone') ? "has-error" : null?>">
                    ...
                    <input type="text" name="phone" id="phone" class="form-control" value="<?= set_value('phone')?>" autocomplete="off" placeholder="Masukan Nomor Phone Supplier">
                    <span class="text-red"><?= form_error('phone')?></span>
                </div>
                <div class="form-group <?= form_error('address') ? "has-error" : null?>">
                    <label for="address">Address :</label>
                    <textarea name="address" id="address" class="form-control" autocomplete="off" placeholder="Masukan Address Supplier"><?= set_value('address')?></textarea>
                    <span class="text-red"><?= form_error('address')?></span>
                </div>
                <div class="form-group <?= form_error('description') ? "has-error" : null?>">
                    <label for="description">Description :</label>
                    <textarea name="description" id="description" class="form-control" autocomplete="off" placeholder="Masukan Description Supplier"><?= set_value('description')?></textarea>
