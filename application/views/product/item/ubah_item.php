...
<section class="content">
    <div class="box">
        <div class="box-body">
            <form action="" method="post" style="margin: 30px 0 30px 0;">
                <div class="form-group <?= form_error('barcode') ? "has-error" : null?>">
                    <input type="hidden" name="item_id" id="item_id" value="<?= $item->item_id?>">
                    <label for="barcode">Barcode :</label>
                    <input type="text" name="barcode" id="barcode" value="<?= set_value('barcode', $item->barcode)?>" autocomplete="off" class="form-control" placeholder="Masukan Barcode">
                    <span class="text-red"><?= form_error('barcode')?></span>
                </div>
                <div class="form-group <?= form_error('name') ? "has-error" : null?>">
                    <label for="name">Name Item :</label>
                    <input type="text" name="name" id="name" value="<?= set_value('name', $item->name)?>" autocomplete="off" class="form-control" placeholder="Masukan Name">
                ...
                <div class="form-group <?= form_error('category') ? "has-error" : null?>">
                    ...
                    <select name="category" id="category" class="form-control">
                        ...
                        <?php foreach($category->result() as $category):?>
                            <option value="<?= $category->category_id?>" <?= $category->category_id == $item->category_id ? "selected" : null ?>><?= $category->name?></option>
                        <?php endforeach;?>
                    </select>
                ...
                <div class="form-group <?= form_error('unit') ? "has-error" : null?>">
                    ...
                    <select name="unit" id="unit" class="form-control">
                        ...
                        <?php foreach($unit->result() as $unit):?>
                            <option value="<?= $unit->unit_id?>"<?= $unit->unit_id == $item->unit_id ? "selected" : null?>><?= $unit->name?></option>
                        <?php endforeach;?>
                    </select>
                ...
                <div class="form-group <?= form_error('price') ? "has-error" : null?>">
                    ...
                    <input type="text" name="price" id="price" value="<?= set_value('price', $item->price)?>" autocomplete="off" class="form-control" placeholder="Masukan Price">
                    <span class="text-red"><?= form_error('price')?></span>
                </div>
                <div class="form-group <?= form_error('stock') ? "has-error" : null?>">
                    <label for="stock">Stock :</label>
                    <input type="text" name="stock" id="stock" value="<?= set_value('stock', $item->stock)?>" autocomplete="off" class="form-control" placeholder="Masukan Stock">
